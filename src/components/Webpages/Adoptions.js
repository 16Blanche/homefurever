...
const Adoptions = () => {
    ...
    const fetchAdoptions = () => {
        axios.get("http://3.24.136.73/api/adoption/pending")
            .then((response) => {
        ...
        axios.get("http://3.24.136.73/api/adoption/active")
            .then((response) => {
    ...
    const handleRejectConfirmation = () => {
        axios.patch(`http://3.24.136.73/api/adoption/decline/${selectedAdoption._id}`, { rejection_reason: rejectionReason === 'Other' ? otherReason : rejectionReason })
            .then(() => {
    ...
    const handleSubmitDate = () => {
        axios.patch(`http://3.24.136.73/api/adoption/approve/${selectedAdoption._id}`, { visitDate, visitTime })
            .then(() => {
                setPendingAdoptions(prev => prev.filter(adopt => adopt._id !== selectedAdoption._id));
    ...
    const handleCompleteAdoption = () => {
        ...
        axios.patch(`http://3.24.136.73/api/adoption/complete/${selectedActiveAdoption._id}`)
            .then(() => {
    ...
    const handleSubmitFailed = () => {
        axios.patch(`http://3.24.136.73/api/adoption/fail/${selectedActiveAdoption._id}`, { reason: failedReason === 'Other' ? otherFailedReason : failedReason })
            .then(() => {
