...
const BarangayTable = () => {
  ...
  const handleEditSave = (row) => {
    ...
    if (editValue !== row[editingCell.columnName]) {
      ...
      axios
        .put(`http://3.24.136.73/api/barangay/update/${row._id}`, { [editingCell.columnName]: editValue })
        .then((response) => {
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    axios.post('http://3.24.136.73/api/barangay/new', formData)
      .then(response => {
  ...
  const handleSubmitImportedData = () => {
    fileData.forEach((row) => {
      ...
      axios.post('http://3.24.136.73/api/barangay/new', formattedRow)
        .then(response => {
          setBarangays(prevBarangays => [...prevBarangays, response.data.savedBarangay]);
          setFilteredBarangays(prevBarangays => [...prevBarangays, response.data.savedBarangay]);
        })
