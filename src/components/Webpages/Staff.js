...
const Staff = () => {
    ...
    useEffect(() => {
        axios.get("http://54.206.91.60/api/staff/id/" + id)
            .then((response) => {
    ...
    const handleDeleteConfirm = (staff) => {
        axios
            .delete(`http://54.206.91.60/api/staff/delete/${staff._id}`, {
                data: { deleteReason }
            })
            .then((response) => {
    ...
    const handleFormSubmit = (e) => {
        e.preventDefault();
        if (validateForm()) {
            axios.post("http://54.206.91.60/api/staff/new", formData)
                .then((response) => {
    ...
    const handleUpdateFormSubmit = (e) => {
        e.preventDefault();
        if (validateForm()) {
            axios.put(`http://54.206.91.60/api/staff/update/${selectedStaffForUpdate._id}`, formData)
                .then((response) => {
