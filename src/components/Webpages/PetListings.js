...
const PetListings =()=>{
    ...
    const handleEditSubmit = (e) => {
        ...
        axios.put(`http://3.24.136.73/api/pet/update/${selectedPet._id}`, updatedPet)
            .then(response => {
    ...
    const handleArchiveSubmit = () => {
        axios.delete(`http://3.24.136.73/api/pet/delete/transfer/${selectedPetForArchive._id}/${archiveReason}`)
            .then((response) => {
    ...
    useEffect(() => {
        axios.delete('http://3.24.136.73/api/pet/delete/:id')
            .then((response) => {
                console.log(response.data.thePet);
            }) 
            .catch((error) => {
    ...
    useEffect(() => {
        axios.get("http://3.24.136.73/api/pet/name/" + pname)
            .then((response) => {
